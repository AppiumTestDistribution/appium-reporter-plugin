...
export class ReportPlugin extends BasePlugin {
    ...
    exclusionList = ['deletesession', 'title', 'screenshot'];
    ...
    async createSession(next,driver,jwpDesCaps,jwpReqCaps,caps) {
        ...
        // await Reporter.createSesssionDataStore(session.value[0]);
        return session;
    }

    async handle(next, driver, commandName, ...args) {
        const result = await next();
        if(!this.exclusionList.includes(commandName.toLowerCase())){
            ...
            img = await img.toFormat("jpeg", { mozjpeg: true })
            .toBuffer()    
            .then(data => {
                console.log('success');
